<script setup>
  ...
  const confirmation = ref({
    fr: 'Le contenu a bien été modifié',
    eng: 'Content has been updated successfully',
  })

  const errMessage = ref({
    fr: 'Une erreur est survenue lors de la modification du contenu',
  ...
  const docRef = doc(db, 'contents', contentId.value)
  const snap = await getDoc(docRef)
  Content.current = Object.assign({}, snap.data(), { id: contentId.value })
  Content.current.activeRadioButton = Content.current.active === true  && 'TRUE' || 'FALSE'
        ...
        <form>
          ...
          <div class="radio-holder-group">
            <div class="radio-holder">
              <input id="content_active_true" type="radio" name="active" value="TRUE" v-model="Content.current.activeRadioButton">              
